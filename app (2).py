...
def call_groq(prompt):
    ...
    try:
        ...
        return None, f"Groq HTTP {r.status_code}: {r.text[:50]}"
    except Exception as e: return None, f"Groq Erro: {str(e)[:30]}"
    ...
    for url_base in endpoints:
        try:
            ...
            if r.ok:
                ...
                st.session_state.last_errors.append(f"Gemini 404/Erro em {url_base.split('/')[-2]}")
        except: continue
    return None, "Gemini falhou em todos os endpoints"
...
def generate_texts(products, tom_texto):
    ...
    texts, provider = res, err1 if not isinstance(err1, str) else "IA"
    if isinstance(res, tuple): texts, provider = res
    ...
    if st.button("🚀 Gerar Ofertas"):
        ...
        if keywords:
            with st.status("Processando...", expanded=True) as status:
                ...
                else: status.update(label="Nada novo encontrado.", state="error")
