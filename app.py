...
GROQ_API_KEY = st.secrets.get("GROQ_API_KEY") # Nova chave opcional
...
def generate_texts(products, tom_texto):
    ...
    for idx, p in enumerate(products, 1):
        prompt += f"{idx}. {p['productName'][:50]} | R${p['priceMin']} | {int(float(p['priceDiscountRate']))}% OFF | {p['offerLink']}\n"

    # Tenta Groq primeiro, depois Gemini
    res = call_groq(prompt)
    if not res: res = call_gemini(prompt)
    
    if res:
        texts, provider = res
        while len(texts) < len(products): texts.append("⚠️ Erro na geração.")
        return texts[:len(products)], provider
    return ["❌ Falha em todas as IAs"] * len(products), "Nenhuma"
...
with st.container():
    ...
    with st.expander("⚙️ Configurações"):
        ...
        if not GROQ_API_KEY: st.info("💡 Dica: Adicione 'GROQ_API_KEY' nos Secrets para maior velocidade e estabilidade.")

    if st.button("🚀 Gerar Ofertas"):
        keywords = [k.strip() for k in kws.split('\n') if k.strip()]
        if keywords:
            with st.status("Buscando e criando...", expanded=True) as status:
                all_p = []
                ...
                if sel:
                    ...
                    status.update(label=f"Sucesso com {prov}!", state="complete", expanded=False)
...
if st.session_state.history:
    ...
    for item in st.session_state.history:
        with st.container():
            st.markdown(f"""<div class='card'><div style='display: flex; justify-content: space-between;'><div><div class='shopee-orange'>{item['name'][:60]}...</div><div style='margin-top: 8px;'><span class='price-tag'>R$ {item['price']}</span> <span style='color: #888; font-size: 0.8em;'>• {item['origin']}</span> <span class='ai-badge'>{item['provider']}</span></div></div><div class='discount-badge'>{item['discount']}% OFF</div></div></div>""", unsafe_allow_html=True)
